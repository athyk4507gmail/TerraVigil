...
const api = {
  ...
  // NEW: API function to detect illegal mining
  detectIllegalMining: async (miningGeoJSON, boundaryFile) => {
    const formData = new FormData();
    const geojson = miningGeoJSON?.geojson ? miningGeoJSON.geojson : miningGeoJSON;
    formData.append('mining_geojson_file', new Blob([JSON.stringify(geojson)], { type: 'application/json' }), 'mining_geojson.json');
    if (boundaryFile) {
      formData.append('boundary_file', boundaryFile);
    }
    const response = await axios.post(`${BACKEND_URL}/illegal_mining`, formData);
    return response.data;
  },
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-6">
          ...
          <div className="text-3xl text-white mt-2">{latestAnalysis?.area_ha ? (latestAnalysis.area_ha / 100).toFixed(3) : '—'} km²</div>
        </div>
        <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-6">
      ...
      {(latestAnalysis?.legal_area_ha !== undefined || latestAnalysis?.illegal_area_ha !== undefined) && (
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-gray-900/50 border border-green-500/30 rounded-lg p-6">
            <div className="text-green-400 text-sm">Legal Mining Area</div>
            <div className="text-3xl text-green-400 mt-2">{latestAnalysis?.legal_area_ha ? (latestAnalysis.legal_area_ha / 100).toFixed(3) : '0.000'} km²</div>
          </div>
          <div className="bg-gray-900/50 border border-red-500/30 rounded-lg p-6">
            <div className="text-red-400 text-sm">Illegal Mining Area</div>
            <div className="text-3xl text-red-400 mt-2">{latestAnalysis?.illegal_area_ha ? (latestAnalysis.illegal_area_ha / 100).toFixed(3) : '0.000'} km²</div>
          </div>
        </div>
      )}
    ...
    <div>
      ...
      <div className="h-[75vh] w-full rounded-lg border border-gray-800 overflow-hidden">
        <MapContainer center={[20.5937, 78.9629]} zoom={5} style={{ height: '100%', width: '100%' }}>
          ...
          {/* Show legal mining in green */}
          {latestAnalysis?.legal_geojson && (
            <GeoJSON data={latestAnalysis.legal_geojson} style={{ color: '#00FF00', weight: 3, fillOpacity: 0.3 }} />
          )}
          {/* Show illegal mining in red */}
          {latestAnalysis?.illegal_geojson && (
            <GeoJSON data={latestAnalysis.illegal_geojson} style={{ color: '#FF0000', weight: 3, fillOpacity: 0.3 }} />
          )}
        </MapContainer>
      </div>
      {!latestAnalysis?.geojson && (
      ...
      {latestAnalysis?.legal_geojson && latestAnalysis?.illegal_geojson && (
        <div className="mt-4 flex space-x-4">
          <div className="flex items-center space-x-2">
            <div className="w-4 h-4 bg-green-500 rounded"></div>
            <span className="text-sm text-gray-300">Legal Mining</span>
          </div>
          <div className="flex items-center space-x-2">
            <div className="w-4 h-4 bg-red-500 rounded"></div>
            <span className="text-sm text-gray-300">Illegal Mining</span>
          </div>
        </div>
      )}
    </div>
...
// UPDATED: New workflow for multi-step analysis with illegal mining detection
function ReportsView({ setThreeDData, setActiveView, setLatestAnalysis }) {
    ...
    const [boundaryFile, setBoundaryFile] = useState(null); // New state for boundary file
    const [isLoading, setIsLoading] = useState(false);
    const [analysisResults, setAnalysisResults] = useState(null); // Results from step 1
    const [illegalMiningResults, setIllegalMiningResults] = useState(null); // Results from illegal mining detection
    ...
    const handleDetectIllegalMining = async () => {
        if (!analysisResults) {
            alert("No mining analysis results available. Please run mining detection first.");
            return;
        }
        setIsLoading(true);
        try {
            const results = await api.detectIllegalMining(analysisResults, boundaryFile);
            setIllegalMiningResults(results);
            // Update the latest analysis to include legal/illegal data for map display
            const updatedAnalysis = {
                ...analysisResults,
                legal_geojson: results.legal_geojson,
                illegal_geojson: results.illegal_geojson,
                legal_area_ha: results.legal_area_ha,
                illegal_area_ha: results.illegal_area_ha
            };
            setLatestAnalysis(updatedAnalysis);
            setStep(3); // Move to step 3 to show results
        } catch (error) {
            console.error('Illegal mining detection failed:', error);
            const msg = error?.response?.data?.detail || error?.message || 'Illegal mining detection failed';
            alert(msg);
        }
        setIsLoading(false);
    };

    const handleSkipIllegalDetection = () => {
        setStep(3); // Skip to volume estimation
    };
    ...
    return (
        <div>
            ...
            {/* Step 2: Detect Legal/Illegal Mining */}
            {step === 2 && (
                <div className="bg-gray-900/50 border border-gray-800 p-8 rounded-lg max-w-lg mx-auto text-center">
                    <h3 className="text-xl font-semibold mb-2 text-white">Step 2: Detect Legal/Illegal Mining</h3>
                    ...
                    <p className="mb-6 text-gray-400">Upload boundary files to determine if mining is legal or illegal, or skip to volume estimation.</p>
                     
                     <div className="mb-8 text-left">
                        <label className="block mb-2 text-sm font-medium text-gray-400">Boundary File (.zip, .geojson)</label>
                        <input type="file" accept=".zip,.geojson" onChange={(e) => setBoundaryFile(e.target.files[0])} 
                               className="w-full p-3 bg-gray-800 border border-gray-700 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-[#00BFFF]"/>
                        <p className="text-xs text-gray-500 mt-1">Upload legal mining boundary files (shapefile as .zip or GeoJSON)</p>
                    </div>
                    
                    <div className="space-y-3">
                        <button onClick={handleDetectIllegalMining} disabled={isLoading || !boundaryFile} 
                                className="w-full bg-red-600 text-white py-3 px-6 rounded-md font-medium hover:bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                            {isLoading ? 'Detecting...' : 'Detect Legal/Illegal Mining'}
                        </button>
                        
                        <button onClick={handleSkipIllegalDetection} disabled={isLoading}
                                className="w-full bg-gray-700 text-white py-2 px-6 rounded-md font-medium hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                            Skip & Proceed to Volume Estimation
                        </button>
            ...
            {step === 3 && (
                <div className="bg-gray-900/50 border border-gray-800 p-8 rounded-lg max-w-lg mx-auto text-center">
                    <h3 className="text-xl font-semibold mb-2 text-white">Step 3: Volume Estimation & 3D Visualization</h3>
                    {illegalMiningResults && (
                        <div className="mb-6 p-4 bg-gray-800 rounded-lg text-left">
                            <h4 className="font-semibold text-white mb-2">Legal/Illegal Mining Results:</h4>
                            <p className="text-green-400 text-sm">Legal Mining: {illegalMiningResults.legal_area_ha?.toFixed(2)} ha</p>
                            <p className="text-red-400 text-sm">Illegal Mining: {illegalMiningResults.illegal_area_ha?.toFixed(2)} ha</p>
                        </div>
                    )}
